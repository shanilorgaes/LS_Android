...
import com.orgaes.ls.Activity_LuckLoser;
...
public class Winners_Adapter_LOOSERS extends RecyclerView.Adapter<Winners_Adapter_LOOSERS.MyViewHolder> {
    Dialog dialog;
    Runnable runnableluck;
    Handler handlerluck;
    Context context;
    Activity_LuckLoser activity_luckLoser;
    String auth_token;
    ...
    public Winners_Adapter_LOOSERS(ProgressBar progressBar, String auth_token, List<Draw_Winners_Winner> draw_winners_winners, Context applicationContext, Activity_LuckLoser activity_luckLoser) {
    ...
    @NonNull
    @Override
    public Winners_Adapter_LOOSERS.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.winner_sub_layout, parent, false);

        return new Winners_Adapter_LOOSERS.MyViewHolder(itemView);
    }

    @SuppressLint("SetTextI18n")
    @Override
    public void onBindViewHolder(@NonNull Winners_Adapter_LOOSERS.MyViewHolder holder, int position) {
        ...
        holder.userpic.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                final MediaPlayer mp = MediaPlayer.create(context, R.raw.btn_click);
                mp.start();
                ProfilePopup(draw_winners_winners.get(position).getUser_id(),"other_User");
            }
        });
...
//                                + v.getTag(), Toast.LENGTH_SHORT).show();
        ...
        Duration.setText(""+todaysLuck_homes.get(0).getDuration()+context.getString(R.string.sec));
    ...
    private void ProfilePopup(String USERID, String other_user) {
        if (clicked1==0){
            clicked1=1;
            dialog = new Dialog(activity_luckLoser, android.R.style.Theme_Dialog);
            dialog.getWindow().requestFeature(Window.FEATURE_NO_TITLE);
